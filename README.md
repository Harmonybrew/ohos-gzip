...
本项目为 OpenHarmony 平台编译了 gzip，并发布预构建包。

## 获取预构建包

前往 [release 页面](https://github.com/Harmonybrew/ohos-gzip/releases) 获取。
...
export PATH=~/gzip-1.14-ohos-arm64/bin:$PATH
...
示例：
```sh
hdc file send gzip-1.14-ohos-arm64.tar.gz /data
hdc shell

cd /data
tar -zxf gzip-1.14-ohos-arm64.tar.gz
export PATH=/data/gzip-1.14-ohos-arm64/bin:$PATH
...
这个项目使用本地编译（native compilation，也可以叫本机编译或原生编译）的做法来编译鸿蒙版 gzip，而不是交叉编译。

需要在 [鸿蒙容器](https://github.com/hqzing/dockerharmony) 中运行项目里的 build.sh，以实现 gzip 的本地编译。
