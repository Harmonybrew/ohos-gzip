...
cd gzip-1.14
./configure --prefix=/opt/gzip-1.14-ohos-arm64
make -j$(nproc)
make install
cd ..
...
cp gzip-1.14/COPYING /opt/gzip-1.14-ohos-arm64/
cp gzip-1.14/AUTHORS /opt/gzip-1.14-ohos-arm64/

# 打包最终产物
cp -r /opt/gzip-1.14-ohos-arm64 ./
tar -zcf gzip-1.14-ohos-arm64.tar.gz gzip-1.14-ohos-arm64
